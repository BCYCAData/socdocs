...
{#if open}
	...
	></button>
	<aside
		class="fixed inset-y-0 left-0 z-50 w-80 overflow-auto border-r border-surface-200 bg-white p-4 lg:hidden dark:border-surface-700 dark:bg-surface-900"
	>
		<div class="mb-4 flex items-center justify-between">
			<h2 class="font-semibold">{guide.title}</h2>
			<button
				type="button"
				class="rounded border border-surface-300 px-2 py-1 text-sm dark:border-surface-700"
